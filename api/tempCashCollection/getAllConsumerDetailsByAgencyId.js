...
function getAllConsumerDetailsByAgencyId(box, nextFunc) {
  ...
  var sql = "SELECT TBD.*,U.agency_id,U.email_id,U.mobile_number,CONCAT(U.first_name,' ',U.last_name) as username FROM temp_bill_details as TBD LEFT JOIN agencies as A ON A.id = TBD.agency_id LEFT JOIN consumer_accounts as CA ON CA.consumer_account_number = TBD.consumer_account_number LEFT JOIN users as U ON U.user_id = CA.user_id WHERE TBD.agency_id = '"+box.reqParams.agency_id+"' AND TBD.payment_status != 'Full Payment' order by bill_id desc LIMIT 250";
  console.info(sql);
  con.query(sql,
  function (err, result) {
        if (err){
