...
function getOneConsumerDetailsByAgencyIdAndConsumerNumber(box, nextFunc) {
  ...
  var sql = "SELECT TBD.*,CA.average_consumption,CA.previous_reading,U.agency_id,CONCAT(U.first_name,' ',U.last_name) as username,AG.name as agency_name,U.mobile_number,U.address_line_1,U.address_line_2,U.city,U.pin_code,S.name as state_name,C.name as country_name,CA.sub_division_id as consumer_sub_division_id FROM temp_bill_details as TBD LEFT JOIN agencies as AG ON AG.id = TBD.agency_id LEFT JOIN consumer_accounts as CA ON CA.consumer_account_number = TBD.consumer_account_number LEFT JOIN users as U ON U.user_id = CA.user_id LEFT JOIN states as S ON U.state_id = S.id LEFT JOIN countries as C ON S.country_id = C.id WHERE TBD.agency_id = "+box.reqParams.agency_id+" AND TBD.bill_id = '"+box.reqParams.bill_id+"' AND TBD.payment_status = 'Not Paid' and bill_status='active' ";
  con.query(sql,
  function (err, result) {
        if (err){
          console.info(err)
