...
function postUtilities(box, nextFunc) {
  var whoAmI = postUtilities.name;
      ...
      if(result_validate.length >= 1){
        ...
        con.query(sql,inserting_data,function (err, result) {
          if (err){
            ...
            return nextFunc(
              new ActErr(whoAmI, ActErr.DBOperationFailed, 'postUtilities', err)
              );
          }else{
