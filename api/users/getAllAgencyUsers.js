...
function getAllUsers(box, nextFunc) {
  ...
  var sql = "SELECT u.user_id,u.status,uuid,first_name,last_name,email_id,mobile_number,date_of_birth,u.address_line_1,u.address_line_2,u.city,s.name as state,u.pin_code,c.name as country,user_role,ag.name as agency_name,u.created_by FROM users u ";
      sql += "LEFT JOIN states as s ON s.id = u.state_id LEFT JOIN countries as c ON c.id = u.country_id LEFT JOIN agencies ag ON ag.id = u.agency_id where ag.id = '"+box.reqParams.agency_id+"' AND u.user_type = 'agency' AND u.status = 'active'";
  con.query(sql,
  function (err, result) {
        if (err){
          console.info(err)
