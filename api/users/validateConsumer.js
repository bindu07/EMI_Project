...
function getUserById(box, nextFunc) {
  ...
  var sql = "SELECT U.user_id,U.email_id,U.mobile_number,U.user_role,U.date_of_birth,U.agency_id,CONCAT(U.first_name,'_',U.last_name) as username,SD.sub_division_name,AG.name as agency_name FROM users as U LEFT JOIN consumer_accounts CA ON CA.user_id = U.user_id "+UserMappingToSubDivision+" WHERE CA.consumer_account_number = '"+box.reqBody.consumer_account_number+"' AND U.password = '"+sha1(box.reqBody.password)+"' AND U.status = 'active' GROUP BY U.user_id";
  console.info(sql);
  con.query(sql,
  function (err, result) {
        if (err){
